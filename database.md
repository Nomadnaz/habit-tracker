...
- Date columns: zero-padded ISO `YYYY-MM-DD`, 1-indexed months, user's local timezone, stored as **`TEXT`, not native `DATE`** — see system-model.md. This is deliberate, not an oversight: an audit (2026-07-06) suggested new tables use native `DATE` columns, but Postgres `DATE`/`TIMESTAMPTZ` types invite exactly the UTC-vs-local mismatch bug class that was the single largest fix in that same audit (client code mixing `new Date(dateKey)` — parsed as UTC midnight — with locally-formatted date keys). Keeping dates as exact TEXT strings the client controls end-to-end is what makes that bug class impossible at the DB layer. Every existing table already does this consistently — don't introduce a second convention for new ones.
- Primary keys: every existing table uses `id TEXT PRIMARY KEY` populated by a client-generated id, not a native `UUID` column — this stays the convention (changing column type would be a breaking migration across ~20 live tables for no functional gain, since `TEXT` happily stores a UUID-formatted string). What IS worth fixing: 13 files under `lib/` each redefine their own copy of the same weak `genId()` (`Math.random().toString(36) + Date.now().toString(36)`) instead of sharing one helper backed by `crypto.randomUUID()` (already used server-side, e.g. `_shared/actionExecutor.ts`'s `create_task`). Consolidating those 13 copies into one `lib/id.ts` using `crypto.randomUUID()` is a good follow-up — still writing into the same `TEXT` column, just a stronger id generator — but touches enough call sites that it should be its own task, not bundled here.
